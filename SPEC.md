...
- **`Enemy`** — Mirror, constrained to cols 4–7 (`col ∈ [COLS/2, COLS - 1]`), hp 100. Composes a `Caster(Team.ENEMY)` (its deck is empty today — only the `basicAttack` slot is set to `wind_slash`) and a `GridPosition`. Inherits no class — same composition pattern as Player. Placeholder AI: `stepRandomly()` picks a random direction every 0.5–1.5s. `PlayState.enemyAi()` loops over every enemy in the list and fires that enemy's held `basicAttack` through the unified pipeline every time its deck cooldown is up — each enemy carries its own `attackTimer`/`attackInterval`, so cadences are independent. **Status & death are wired**: `applyFreeze(duration)` halts the cooldown gate and tints the sprite blue (`FREEZE_TINT`); `isFrozen()` is the gate read by `PlayState.enemyAi`. When `hp <= 0` the enemy enters a 0.5s death fade (`isDying()` → sprite + shadow alpha lerp); `isDead()` reports when it's safe to remove. `render(batch, font)` also draws a small HP number centered just under the sprite (alpha fades with the death animation). Dead enemies stay in the list — render/shadow/AI all early-out on `isDead()` so they become inert without growing skill-side complexity.
...
- **`StrikeInstance`** — Phases: `DASH_FORWARD` (0.10s) → `HIT` (0.20s) → `DASH_BACK` (0.10s) → `DONE`. Acquires the caster's `InputLock`, snaps to `originCol+1` via `pos.setTile(...)` (unclamped), spawns a `ClashEffect` on the target tile (`originCol+2`), applies `DAMAGE`-typed effects to whichever enemy is standing on the target tile (`ctx.enemyAt(targetCol, row)` — `null` if empty), then snaps back. The VFX anchors to the *tile* (`ctx.projectedTileWorld` + `ctx.tileDepthScale`) so the flourish still lands even when no one's there to take it. Single-target tile lookup; assumes player firing. Status-effect TODOs (`POISON/BURN/STUN`) are flagged inline.
- **`ProjectileInstance`** — Two movement modes via `ProjectileConfig`. **Team-aware**: `dir = caster.getTeam() == PLAYER ? +1 : -1`, applied to spawn-offset, velocity sign, grid-exit edge, sprite horizontal flip, and target lookup.
    - `STRAIGHT`: spawns at the caster's visual position and travels `dir * config.speed`; finishes when it crosses the appropriate grid edge (right for player, left for enemy) or **overlaps the first opposing entity's tile center on the same row**. Player-fired: iterates `ctx.enemiesOnRow(row)` and stops on the first one within `halfW` — a closer enemy soaks the hit so the one behind survives. Enemy-fired: still single-target against `ctx.player`.
    - `LOB`: parabolic arc from caster to `originCol + targetRange * dir` over `LOB_FLIGHT_TIME` (0.50s); on landing applies damage via `ctx.enemyAt(landCol, row)` — only the enemy standing on the landing tile takes the hit. The follow-up `ZoneInstance` "cloud" is then spawned via `ctx.combatSystem.spawn(...)` for lingering effect (tile-pinned, so subsequent ticks affect whichever enemy walks into the cloud). LOB-from-enemy is latent (no enemy fires LOB skills today).
...
- **`BeamInstance`** — Phases: `CHARGE` (0.20s) → `FIRE` (0.70s) → `FADE` (0.25s) → `DONE`. Acquires `InputLock` for the windup; releases at the start of `FADE`. On entering `FIRE` it **pierces every alive enemy** on the caster's row past `originCol` (one application each, guarded by the `hitApplied` flag so the loop runs exactly once per fire). Render: if `Skill.vfxAnimation` is present (e.g. `ice_beam`'s sprite-sheet) the beam draws the current `Animation` frame; otherwise it stretches the static `vfxTexture`. During `CHARGE` the beam extends out from the caster (`w = fullW * (phaseTime/CHARGE)`); during `FADE` alpha lerps to zero.
...
- **`ZoneInstance`** — Phases: `APPEAR` (0.15s) → `ACTIVE` (1.00s) → `FADE` (0.25s) → `DONE`. Pinned to a target tile (defaults to `caster +1, sameRow`; can be overridden — that's how `ProjectileInstance` plants a cloud on its landing tile). Every `TICK_INTERVAL` (0.33s) it resolves the target via `ctx.enemyAt(targetCol, targetRow)` and applies effects — so an enemy walking off the tile escapes subsequent ticks, and an enemy walking onto it gets ticked from then on. Returns `true` from `isRenderUnder()`, so it draws under entities (terrain feel).
...
### Systems
- **`BattleContext`** — bag of references (battlefield, player, `List<Enemy> enemies`, `List<BattleVfx> vfx`, `GameEnvironment environment`, the shared `clashTexture` (`effects/clash.png`, used by `CombatSystem.resolveProjectileClashes` to spawn the cancel flourish), *and* a back-reference to its owning `combatSystem` set immediately after construction). Constructed once in `PlayState`. Exposes two target-lookup helpers used by every skill instance: `enemyAt(col, row)` returns the first alive enemy on that exact tile (or `null`) — used by Strike, Zone, and Projectile LOB landing; `enemiesOnRow(row)` returns a fresh list of alive enemies on a row — used by Beam (apply to all) and Projectile straight (stop on first overlap). Projection helpers `projectedTileWorld(col,row)` (tile center in 2D viewport space) and `tileDepthScale(row)` (1.0 near → ~0.78 far) are how skill instances place their VFX correctly on the 3D-projected floor. `buildCache()` (called from `PlayState.resize`) bakes a `COLS×ROWS` lookup so every per-frame skill draw is a constant-time array hit, not a `cam3D.project()` call. Pass-around context (not a service locator) so skills can be unit-tested with fakes.
...
- **Hit resolution lives inside each `SkillInstance`** (synchronous, fixed-timing). There's no separate collision system — every projectile, beam, strike, and zone does its own tile-center overlap test inside its `SkillInstance` subclass. Multi-enemy lookups go through `BattleContext.enemyAt` / `enemiesOnRow` so the targeting semantics live in one place per shape rather than in a shared collision sweep. `ProjectileInstance` is also team-aware: player-fired projectiles consult those helpers, enemy-fired ones still target `ctx.player` directly.
...
- **Strike/Beam/Aura/Zone assume player firing.** They take a `Caster` now and use the multi-enemy `ctx.enemyAt` / `ctx.enemiesOnRow` helpers, but the lookups all hunt for `Enemy`s and use player-relative directions. If enemies ever fire those shapes, each needs the same team-aware target lookup `ProjectileInstance` got (pick `Enemy` vs `Player` based on `caster.getTeam()`).
...
- **Multi-enemy support landed; encounter design hasn't.** `PlayState` now holds a `List<Enemy>` (seeded with two today) and skill instances use `ctx.enemyAt` / `ctx.enemiesOnRow` for tile/row lookups. What's still missing is anything that *uses* it: no spawner, no encounter definition, no varied enemy stats or sprites, no waves. The list count is hardcoded in `PlayState`'s constructor.
