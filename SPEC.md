...
│       │   ├── RewardOffer.java        # titled set of same-type options; skillOffer() factory
...
screens, screens.state  →  systems, systems.combat, environment, entities, components, sessions, ui, skills, input, registry, rewards
rewards                 →  skills, sessions, ui       (options wrap Skills, apply() mutates the GameSession, cards build on the ui style kit)
...
2. On win, each enemy's 0.5s death fade plays first (`isDying`) — the fade is armed by `Combatant.onDeath()` which `DamageSystem` fires when HP hits 0; after every enemy's `isDead()` flips true, `progressPlayer()` + `resetStaging()` run, then `RewardOffer.skillOffer(session)` rolls up to 3 unowned skills and the transition fires via `Main.setScreen(new RewardScreen(game, List.of(offer)))` — or straight to `OverworldScreen` when the offer is null (pool drained).
3. `RewardScreen` walks its offer queue: pick (apply to the deck) or skip each offer, then `Main.setScreen(new OverworldScreen(...))`. A future evolution unlock appends an `evolutionOffer` to the same queue.
...
- **Rewards are skills-only.** The `RewardOption`/`RewardOffer` abstraction is in place, but only `SkillRewardOption` + `RewardOffer.skillOffer` exist. Evolutions (the `registry/` evolution types are unwired) and traits (passive abilities — not yet designed) become sibling option classes + offer factories appended to the `RewardScreen` queue. Offers are always skippable today; a `skippable` flag waits for the first mandatory offer type.
