...
- **HUDs** borrow the shared `GeneratedAssets.pixel()` (constructor-injected) — they own no textures and have no `dispose()`. `SkillSelectOverlay` also borrows the pixel, but owns its Scene2D `Stage` and FreeType-generated fonts; `SkillSelectState.dispose()` calls its `dispose()`. `Battlefield` and `StatusContainer`s likewise hold no GPU resources.
