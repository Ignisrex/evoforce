...
- **Screen → State** layering (battle only): `GameScreen` runs a `GameScreenState` machine (`PlayState` ↔ `SkillSelectState`), owns the `Encounter` + HUDs, and reads the run off `game.session`. Per-caster state (deck/slots/basic attack) lives on the `Caster`; the catalogue + player loadout live on `GameSession`, not `GameScreen`. The state machine is not a stack.
...
- **Pragmatic component-style, not ECS.** `Player`/`Enemy` are concrete classes composing small components (`Caster`, `GridMovement`, `Health`, `Stats`, `StatusContainer`, …). Cross-cutting behavior goes in `systems/` (`Encounter`, `BattleState`, `CombatSystem`, `MovementSystem`, `ai/EnemyAi`). Ashley is on the classpath but not used — don't reach for it.
- **Skill execution.** `Skill` is data; `SkillFactory.create(Skill, Combatant)` switches on `Shape` to produce a runtime `SkillInstance`. `CombatSystem` ticks active instances and prunes finished ones. An instance stores no context: the sim arrives as a `SkillContext` on `update`, rendering as a `RenderContext` on `render`, so it can reach neither the camera while ticking nor the battle while drawing. All five shapes — `Strike/Projectile/Beam/Aura/Zone` — are implemented, each sequenced by its own phase timeline (e.g. Strike `DASH→HIT→DASH_BACK`, Beam `CHARGE→FIRE→FADE`).
- **Two coexisting meters, distinct jobs.** `ManaPool` is the budget bounding *how much you can stage* (committing drains the whole bar, cancelling half); per-skill `SkillCooldowns` gates *which cards appear in the hand and which slots can fire*. Don't conflate them. `ChargeMeter` is dormant, replaced by `ManaPool`.
- **Combat is measured in grid space, never screen space.** Projectiles carry a continuous *column*; hit tests round it and ask `BattleState`. Projection happens only in `render`, through `RenderContext.tileWorld`. If you find yourself reading the camera inside a tick, you are in the wrong place.
- **Three separate action gates.** Movement checks `blocksMovement` (in `MovementSystem`), the basic attack `blocksAttack`, everything else `blocksCasting` — one call site each, via `Combatant.canStartAttack/canStartCast`. Casts route through `CombatSystem.tryCast` (gated) or `cast` (ungated, for already-filtered staged cards).
...
- **World units:** the battlefield is 8 cols × 4 rows; cols 0–3 are player territory, 4–7 enemy. Use the static `Battlefield.floorX/floorZ` for tile→world conversion, and `RenderContext.tileWorld` for tile→screen, rather than recomputing offsets.
